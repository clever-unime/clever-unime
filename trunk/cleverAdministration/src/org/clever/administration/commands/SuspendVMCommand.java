...
public class SuspendVMCommand extends CleverCommand {
  ...
  public void exec(final CommandLine commandLine) {
    try
    {
      ...
      boolean res = (Boolean) ClusterManagerAdministrationTools.instance().execSyncAdminCommand(
              this, target, "VirtualizationManagerAgent", "suspendVm", params, commandLine.hasOption("xml"));

      if (res)
        System.out.println("Virtual Environment successfully paused");
      else
        System.out.println("Failed to pause the Virtual Environment");
